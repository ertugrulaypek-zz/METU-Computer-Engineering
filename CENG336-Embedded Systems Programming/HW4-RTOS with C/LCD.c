...
const char string_pool[2][16] = {" ROSES GOE RED  ", "VIOLETS ARE BLUE"};
...
TASK(LCD)
{
    ...
    while(1)
    {
        ...
        if(getDisplayMode() == 1){
            ...
            LcdPrintString(getFirstRow(),0,0);
            LcdPrintString(getSecondRow(),0,1);
        }
        else{
            
            LcdPrintString(getThirdRow(),0,0);
            LcdPrintString(getFourthRow(),0,1);
        }
        Lcd_refresh();
